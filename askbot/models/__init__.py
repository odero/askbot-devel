    ...
    if getattr(django_settings, 'ENABLE_HAYSTACK_SEARCH', False):
        ...
        if 'postgresql_psycopg2' in askbot.get_database_engine_name():
            ...
            return users_query_set.filter(
                models.Q(userprofile__username__icontains=search_query) |
                models.Q(userprofile__about__icontains=search_query)
            )
...
def user_get_profile_url(self, profile_section=None):
    ...
    url = reverse(
            'user_profile',
            kwargs={'id': self.id, 'slug': slugify(self.userprofile.username)}
        )
    if profile_section:
...
def get_profile_link(self, text=None):
    profile_link = u'<a href="%s">%s</a>' \
        % (self.get_profile_url(), escape(text or self.userprofile.username))
